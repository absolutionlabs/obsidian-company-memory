...
Every session ends with the `close-obsidian-project` skill (installed from its own zip on the same GitHub Release as the main vault-setup skill; see `companion-skills/README.md` for per-tool detail). The protocol the skill executes:
